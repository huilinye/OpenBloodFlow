...
void FixBondCreate::post_integrate()
{
  ...
  double **x = atom->x;
  double dis,disx,disy,disz,nenergy,energytotal;
  tagint *tag = atom->tag;
  tagint **bond_atom = atom->bond_atom;
  int *num_bond = atom->num_bond;
  ...
  //random probability initial in lammps
  ...
  // check the create probability 
  
  stringstream output_filename;
  output_filename << "createPro.dat";
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (fraction < 1.0) {
		  ...
		  com_prob = 1.0-exp(-100.0*fraction*exp(-0.7305*pow(dis-1.0,2)));
  ...
  /* if(comm->me == 0 ){
  stringstream output_filename;
  output_filename << "createbond.dat";
  ofstream output_file;
  
  /// Open file
  
  output_file.open(output_filename.str().c_str(),ofstream::app);
  output_file <<createcount<<" "<<createcounttotal<<" "<<energytotal<<"\n";
  } */
