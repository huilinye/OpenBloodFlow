  ...
  Fix(lmp, narg, arg)
{
  ...
  fraction = 1.0;            //Set the default value if there is no explicit definition in argument of input file
  int seed = 12345;
...
void FixBondBreak::post_integrate()
{
  ...
  double **x = atom->x;
  double disx,disy,disz,dis,nenergy,energytotal;
  tagint *tag = atom->tag;
  int *mask = atom->mask;
 ...
 /*  //ligand-receptor adhesion model            By Huilin Ye 2/28/2017
  // check the break probability 
  
  stringstream output_filename;
  output_filename << "breakPro.dat";
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    if (fraction < 1.0) {
		  ...
		  disy = x[i][1] - x[j][1];
		  disz = x[i][2] - x[j][2];
		  domain->minimum_image(disx,disy,disz);
		  dis = sqrt(disx*disx+disy*disy+disz*disz);
		  
		  
		  com_prob = 1.0-exp(-fraction*exp(0.7305*pow(dis-1.0,2)));
		  /* output_file <<com_prob<<" "<<dis<< "\n"; */
		  
		  //com_prob = 0.5;
      if (tag[i] < tag[j]) {
        if (probability[i] >= com_prob) continue;
      } 
	  else {
        if (probability[j] >= com_prob) continue;
      }
    }
  ...
  MPI_Allreduce(&nenergy,&energytotal,1,MPI_DOUBLE,MPI_SUM,world);
  breakcounttotal += breakcount;
  
  /* if(comm->me == 0){
  stringstream output_filename;
  output_filename << "breakbond.dat";
  ofstream output_file;
  
  /// Open file
  
  output_file.open(output_filename.str().c_str(),ofstream::app);
  output_file <<breakcount<<" "<<breakcounttotal<<" "<<energytotal<<"\n";
  } */
